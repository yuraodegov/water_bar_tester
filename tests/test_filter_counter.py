...
После серии наливов — накопленный filter_ml должен >= target.
"""
...
class TestFilterCounter(BaseTest):
    ...
    def run(self) -> TestResult:
        ...
        if val is None:
            return self._fail("get_counter filter → None")
        ...
        if val >= filter_max_ml:
            return self._fail(f"Filter {val/1000:.1f}L >= max {filter_max_l}L — замените фильтр!", data)
